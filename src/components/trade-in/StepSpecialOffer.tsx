import { Sparkles, Gift, ShieldCheck, ArrowLeft, TrendingUp, Tag, Banknote } from "lucide-react";
...
interface Props {
  ...
  /** "trade" mostra bônus de troca; "sale" mostra bônus de venda em dinheiro. */
  flowType: "trade" | "sale";
  onBack: () => void;
  onContinue: () => void;
}
 ...
 * quando há bônus configurado para o fluxo escolhido (troca OU venda).
 * Reforça o ganho extra para criar tensão positiva antes de pedir o IMEI.
 */
export function StepSpecialOffer({ baseValue, bonusPercent, flowType, onBack, onContinue }: Props) {
  ...
  const bonusLabel = isSale ? "Bônus de venda" : "Bônus de troca";
  const totalLabel = isSale ? "Total a receber em dinheiro" : "Total para usar na sua troca";
  const totalSubLabel = isSale ? "Pago via PIX ou transferência" : "Em crédito imediato na loja";
  const headline = isSale
    ? "Você acabou de desbloquear um bônus em dinheiro"
    : "Você acabou de desbloquear um bônus exclusivo";
  const subheadline = isSale
    ? "Confirme o IMEI agora para garantir o valor com bônus na sua venda."
    : "Confirme o IMEI agora para garantir o valor com bônus aplicado na sua troca.";

  const TotalIcon = isSale ? Banknote : Gift;
        ...
        <div className="relative space-y-5">
            ...
            <div className="flex items-center justify-between gap-3 rounded-2xl border border-accent/40 bg-accent/10 p-3">
              <div className="flex items-center gap-2.5">
                <div className="h-9 w-9 rounded-full bg-accent/25 flex items-center justify-center animate-bounce">
                  <TotalIcon className="h-4 w-4 text-accent" />
                </div>
                <div>
                  <p className="text-[10px] font-semibold uppercase tracking-widest text-accent">
                    {bonusLabel} {`+${bonusPercent.toLocaleString("pt-BR")}%`}
                  </p>
          ...
          <div className="flex items-end justify-between gap-3">
            <div>
              ...
              <p className="text-xs text-muted-foreground">{totalSubLabel}</p>
            </div>
